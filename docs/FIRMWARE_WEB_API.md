# Odrive-Wheel — Documentação do Firmware para a Web App

Referência técnica do firmware **Odrive-Wheel** (FFB + ODrive v0.5.6 fundidos) voltada à implementação e manutenção da aplicação web em `Odrive-Wheel/web/`.

**Público:** desenvolvedores da web app, integradores serial/HID, e quem mantém `fieldCatalog.ts`.

**Referências cruzadas:**
- App web: `Odrive-Wheel/web/src/`
- Firmware custom: `Odrive-Wheel/src/`, `Odrive-Wheel/inc/`
- ODrive base: `ODrive-fw-v0.5.6/`
- HTML legado: `Odrive-Wheel/tools/odrive-wheel.html`
- Torque/FFB teoria: `docs/TORQUE_CONTROL.md`
...
**Paths:** qualquer propriedade em `ODrive-fw-v0.5.6/Firmware/odrive-interface.yaml`.
...
`ODrive-fw-v0.5.6/Firmware/odrive-interface.yaml`

A web app mantém um espelho manual em `web/src/features/config/fieldCatalog.ts` (~120 campos).
...
`web/src/features/live/errorDecoder.ts` — bitmasks portados de `odrive-wheel.html`.
...
Ver secção 6 + 9; lista completa em `web/src/domain/commands/commandRegistry.ts`.

---

## 14. Ficheiros-fonte por tema

### Firmware Odrive-Wheel

| Tema | Ficheiros |
|------|-----------|
| Parser OpenFFBoard | `inc/cmdparser.h`, `src/cmdparser.c`, `src/cmd_table.cpp` |
| Router serial | `ODrive-fw-…/Firmware/communication/ascii_protocol.cpp` |
| FFB core | `src/ffb_task.cpp`, `src/HidFFB.cpp`, `src/EffectsCalculator.cpp` |
| EEPROM FFB | `src/eeprom.c`, `inc/eeprom_addresses.h`, `src/PersistentStorage.cpp` |
| GPIO | `src/gpio_inputs.cpp`, `inc/gpio_inputs.h` |
| Bridge | `src/odrive_bridge.cpp`, `inc/odrive_bridge.h` |
| USB | `src/usb_tinyusb_glue.cpp`, `src/usb_descriptors.c` |
| Build | `Odrive-Wheel/Makefile`, `linker/` |

### Web app

| Tema | Ficheiros |
|------|-----------|
| Schema campos | `web/src/features/config/fieldCatalog.ts` |
| Protocolo | `web/src/features/board/BoardProtocol.ts` |
| Serial | `web/src/features/serial/SerialService.ts` |
| Save | `web/src/features/board/unifiedSave.ts`, `useBoardSave.ts`, `fieldApply.ts` |
| Calibração | `web/src/features/calibration/*` |
| Live debug | `web/src/features/live/liveMonitorCatalog.ts`, `errorDecoder.ts` |
| Telemetria | `web/src/features/telemetry/*` |
| HID | `web/src/features/hid/*` |
| Comandos | `web/src/domain/commands/commandRegistry.ts` |
| Refresh | `web/src/app/refreshPolicy.ts` |
...
`tools/odrive-wheel.html` — comportamento de referência para port React; bitmasks de erro e SCHEMA embutido (~linhas 3560–3900, 10500+ live refresh).
